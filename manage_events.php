...
<div class="container" style="margin-top: 1rem;">
    <div class="events-header-section"
        ...
        <div>
            ...
            <p style="color: var(--text-muted); font-size: 0.9rem;">Manage holidays, company events, and special working
                days.</p>
        </div>
        <button class="btn btn-primary" onclick="openAddEventModal()"
            style="padding: 0.75rem 1.5rem; display: flex; align-items: center; gap: 0.5rem; font-weight: 700;">
            ➕ Add New Event
        </button>
    </div>

    <div class="filter-card"
        style="background: rgba(255, 255, 255, 0.4); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); padding: 1.5rem; border-radius: 1rem; border: 1px solid rgba(255, 255, 255, 0.6); margin-bottom: 2rem; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.05), inset 0 0 0 1px rgba(255,255,255,0.4);">
        <form action="" method="GET" style="display: flex; gap: 1rem; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 250px; position: relative;">
                <span
                    style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: var(--text-muted);">🔍</span>
    ...
    <?php if (isset($_GET['success'])): ?>
        <div
            style="background: #dcfce7; color: #16a34a; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem; font-weight: 600; border: 1px solid #bbf7d0;">
    ...
    <?php if (isset($_GET['error']) || isset($error)): ?>
        <div
            style="background: #fee2e2; color: #ef4444; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem; border: 1px solid #fecaca;">
            <?php echo isset($_GET['error']) ? htmlspecialchars($_GET['error']) : ($error ?? ''); ?>
        </div>
    <?php endif; ?>

    <div class="events-table-container"
        style="background: rgba(255, 255, 255, 0.4); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border-radius: 1rem; border: 1px solid rgba(255, 255, 255, 0.6); overflow-x: auto; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.05), inset 0 0 0 1px rgba(255,255,255,0.4);">
            ...
            <thead>
                <tr style="background: rgba(255, 255, 255, 0.2); border-bottom: 1px solid rgba(255, 255, 255, 0.4);">
                    ...
                    <th
                        ...
                        Type</th>
                    <th
                        style="padding: 1.25rem 1rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; font-size: 0.75rem; text-align: center;">
                        Actions</th>
                </tr>
            </thead>
            <tbody>
                        ...
                        <tr style="border-bottom: 1px solid var(--border-color); transition: background 0.2s;"
                            ...
                            <td style="padding: 1rem; font-weight: 700; color: var(--text-main);">
                                ...
                                <div style="font-size: 0.75rem; color: var(--text-muted); font-weight: 400;">
                                    <?php echo date('l', strtotime($e['event_date'])); ?></div>
                            </td>
                            <td style="padding: 1rem;">
                                <div style="font-weight: 700; color: var(--primary-color);">
                                    <?php echo htmlspecialchars($e['title']); ?></div>
                            </td>
                            <td style="padding: 1rem;">
                                <span style="font-size: 0.7rem; font-weight: 800; padding: 0.3rem 0.6rem; border-radius: 2rem; text-transform: uppercase; 
                                    <?php
                                    if ($e['type'] === 'holiday')
                                        echo 'background: #fee2e2; color: #ef4444; border: 1px solid #fecaca;';
                                    elseif ($e['type'] === 'event')
                                        echo 'background: #fef9c3; color: #a16207; border: 1px solid #fef08a;';
                                    elseif ($e['type'] === 'half_day')
                                        echo 'background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd;';
                                    elseif ($e['type'] === 'wfh')
                                        echo 'background: #f3e8ff; color: #9333ea; border: 1px solid #e9d5ff;';
                                    else
                                        echo 'background: #dcfce7; color: #16a34a; border: 1px solid #bbf7d0;';
                                    ?>">
                            ...
                            <td style="padding: 1rem; text-align: center;">
                                <div style="display: flex; gap: 0.5rem; justify-content: center;">
                                    <button class="btn"
                                        style="padding: 0.4rem 0.75rem; font-size: 0.8rem; border-color: var(--primary-color); color: var(--primary-color);"
                                        onclick="openEditEventModal(<?php echo htmlspecialchars(json_encode($e), ENT_QUOTES, 'UTF-8'); ?>)">Edit</button>
                                    <a href="delete_event.php?id=<?php echo $e['id']; ?>" class="btn"
                                        style="padding: 0.4rem 0.75rem; font-size: 0.8rem; border-color: #ef4444; color: #ef4444; text-decoration: none;"
...
<div class="modal-overlay" id="eventModal" onclick="if(event.target===this) closeEventModal()">
    <div class="modal-content" style="max-width: 500px;">
        <h3 id="modalTitle" style="margin-bottom: 1.5rem; color: var(--primary-color); font-size: 1.5rem;">Add New Event
        </h3>
        <form id="eventForm" action="process_event_manage.php" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="title" id="eventTitle" required
                    placeholder="e.g. Annual Meeting or Public Holiday">
            </div>
            ...
            <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                ...
                <button type="button" class="btn" onclick="closeEventModal()"
                    style="flex: 1; padding: 0.85rem;">Cancel</button>
            </div>
        </form>
    </div>
</div>

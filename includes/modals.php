...
<script>
    ...
    function openAdminCancelModal(data) {
        document.getElementById('cancelEmpName').innerText = data.user_name || data.name;
        document.getElementById('cancelLeaveRange').innerText = data.from_date + ' to ' + data.to_date;
        document.getElementById('cancelLeaveId').value = data.id;

        const checklist = document.getElementById('cancelDaysChecklist');
        checklist.innerHTML = '';
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            // Check if this date was approved
            let isApproved = false;
            if (data.status === 'approved' && (!data.approved_dates || data.approved_dates === 'null')) {
                isApproved = true;
            } else if (data.approved_dates) {
                let appDates = data.approved_dates;
                if (typeof appDates === 'string') {
                    try {
                        appDates = JSON.parse(appDates);
                    } catch(e) {}
                }
                if (Array.isArray(appDates) && appDates.includes(dateStr)) {
                    isApproved = true;
                }
            }
            ...
            div.innerHTML = `
                <input type="checkbox" name="approved_days[]" value="${dateStr}" ${checkedAttr}>
                <span>${displayDate}</span>
            `;
            checklist.appendChild(div);
        }

        document.getElementById('adminCancelLeaveModal').classList.add('active');
        document.body.style.overflow = 'hidden';
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (cancelForm) {
            cancelForm.addEventListener('submit', function(e) {
                const checked = this.querySelectorAll('input[name="approved_days[]"]:checked');
                if (checked.length === 0) {
                    if (confirm("You have unchecked all days. This will revert the entire leave request back to pending. Proceed?")) {
                        this.querySelector('input[name="status"]').value = 'pending';
                    } else {
                        e.preventDefault();
                    }
                }
...
<!-- Admin Cancel/Modify Approved Leave Modal -->
<div id="adminCancelLeaveModal" class="modal-overlay">
    <div class="modal-content">
        <span class="close" onclick="closeModal('adminCancelLeaveModal')">&times;</span>
        <h2 style="margin-bottom: 0.5rem; color: var(--primary-color);">Cancel/Modify Approved Days</h2>
        <p style="font-size: 0.9rem; color: var(--text-muted); margin-bottom: 1.5rem;">Uncheck the dates you wish to cancel/remove for this employee.</p>

        <form action="process_leave.php" method="POST" id="cancelForm">
            <input type="hidden" name="leave_id" id="cancelLeaveId">
            <input type="hidden" name="status" value="approved">

            <div>
                <p><strong>Employee:</strong> <span id="cancelEmpName"></span></p>
                <p><strong>Original Range:</strong> <span id="cancelLeaveRange"></span></p>
                <hr style="margin:1rem 0; border:0; border-top:1px solid var(--border-color);">
                
                <div style="margin-bottom: 1.5rem;">
                    <p style="font-weight: 700; margin-bottom: 0.5rem;">Approved Days:</p>
                    <div id="cancelDaysChecklist" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 0.5rem; background: #f8fafc; padding: 1rem; border-radius: 0.5rem; border: 1px solid var(--border-color);">
                        <!-- Checkboxes generated by JS -->
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                <button type="submit" class="btn" style="background:#16a34a; color:white; flex:1; border:none; padding: 1rem; font-weight: 700; border-radius: 0.5rem; cursor: pointer;">
                    Update Approved Days
                </button>
                <button type="button" class="btn" onclick="closeModal('adminCancelLeaveModal')" style="background:#6b7280; color:white; flex:1; border:none; padding: 1rem; font-weight: 700; border-radius: 0.5rem; cursor: pointer;">
                    Close
                </button>
            </div>
        </form>

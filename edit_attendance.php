...
<div class="container" style="max-width: 650px;">
    <div class="card">
        <div
            style="display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem; border-bottom: 1px solid var(--border-color); padding-bottom: 1.5rem;">
            <div
                style="width: 48px; height: 48px; background: var(--primary-color); color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; font-weight: 700;">
            ...
            <div>
                ...
                <p style="color: var(--text-muted); font-size: 0.9rem;">
                    ...
                    (<?php echo htmlspecialchars($record['emp_id']); ?>) &bull;
        ...
        <form method="POST">
            ...
            <div class="form-group" style="margin-bottom: 2rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: var(--text-muted);">Break
                    Time (HH:MM)</label>
                <?php
                ...
                ?>
                <input type="text" name="break_time" value="<?php echo $break_formatted; ?>" placeholder="HH:MM"
                    pattern="^([0-9]+):([0-5][0-9])$"
                    style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.75rem; font-size: 1rem; background: var(--bg-color); color: var(--text-main);">
            </div>

            <div class="form-group" style="margin-bottom: 2rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: var(--text-muted);">Work
                    Mode</label>
                <select name="work_mode" required
                    style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.75rem; font-size: 1rem; background: var(--bg-color); color: var(--text-main);">
                    <option value="WFO" <?php echo ($record['work_mode'] ?? 'WFO') === 'WFO' ? 'selected' : ''; ?>>WFO
                        (Office)</option>
                    <option value="WFH" <?php echo ($record['work_mode'] ?? 'WFO') === 'WFH' ? 'selected' : ''; ?>>WFH
                        (Home)</option>
                </select>
            </div>
            ...
            <div style="display: flex; gap: 1rem; margin-top: 3rem;">
                ...
                <a href="admin_attendance.php" class="btn"
                    style="flex: 1; text-align: center; display: flex; align-items: center; justify-content: center; text-decoration: none; border-color: var(--border-color);">Cancel</a>
            </div>
        </form>
    </div>
</div>

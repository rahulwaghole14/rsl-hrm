...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO users (name, email, mob_no, role, password, emp_id) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$name, $email, $mob_no, $role, $password, $emp_id]);
...
<div style="max-width: 500px; margin: 2rem auto; background: var(--card-bg); padding: 2.5rem; border-radius: 1.5rem; border: 1px solid var(--border-color); box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
    ...
    <?php if ($success): ?>
        <div style="background: #dcfce7; color: #16a34a; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem;"><?php echo $success; ?></div>
    <?php endif; ?>
    ...
    <form method="POST">
        ...
        <p style="text-align: center; margin-top: 1.5rem; font-size: 0.9rem; color: var(--text-muted);">
            Already have an account? <a href="login.php" style="color: var(--primary-color); font-weight: 600; text-decoration: none;">Login</a>
        </p>
    </form>
</div>
